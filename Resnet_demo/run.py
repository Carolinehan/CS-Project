...
def train(datadir, split, modelname):
    resnet = keras.applications.ResNet50(include_top=False, weights=None, input_tensor=Input(shape=(80,80,3)), input_shape=(80,80,3), classes=1)
    for layer in resnet.layers:
        layer.trainable = True
    es = EarlyStopping(monitor='val_acc', patience=15)
    input_tensor = Input((80, 80, 3))
    x = resnet.output
    ...
    history= model.fit(x=train_data, y=train_labels,verbose=1, batch_size=100,epochs=100, callbacks=[es,mc], validation_split=split)
    ...
    file_name = 'test'
    test_data = np.load(os.path.join(datadir, file_name + '_data.npy'), mmap_mode='r')
    test_labels = np.load(os.path.join(datadir, file_name + '_label.npy'), mmap_mode='r')
...
datadir='/data2/team16a/shuffle/'
#train(datadir, 0.32, 'per50000')
train(datadir, 0.6, 'per30000')
train(datadir, 0.86, 'per10000')
train(datadir, 0.32, 'avg50000')
