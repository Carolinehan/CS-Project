...
def normalise(data, mean, std, train=True):
    data = data.astype(np.float32)
    l_mean = mean
    l_std = std
    if train:
        l_mean = np.mean(data)  # compute mean over all pixels make sure equivariance is preserved
        l_std = np.std(data)
    data -= l_mean


    data /= l_std
    return data, l_mean, l_std

def large_data_shuffle(output, mean, std, train=True):
    data,labels = read_npy(output)
    data, mean, std = normalise(data, mean, std, train)
...
mean, std= large_data_shuffle('train', 0.0, 0.0)
large_data_shuffle('test', mean, std, train=False)
