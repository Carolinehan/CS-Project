...
def settings(args):
   ...
   # f2 = h5py.File('validation.hdf5', 'r+')
   # f3 = h5py.File('test.hdf5', 'r+')
   #
   # data = {}
   #
   # data1 = np.array(f1['data']) #numpy format
   # data2 = np.array(f2['data'])
   # data3 = np.array(f3['data'])
   #
   # data['train_x'] = np.reshape(data1, (data1.shape[0],784))
   # data['valid_x'] = np.reshape(data2, (data2.shape[0],784))
   # data['test_x'] = np.reshape(data3, (data3.shape[0],784))
   #
   # labels1 = f1['labels']
   # labels2 = f2['labels']
   # labels3 = f3['labels']
   #
   # data['train_y'] = np.array(labels1)
   # data['valid_y'] = np.array(labels2)
   # data['test_y'] = np.array(labels3)
   #
   # print(data1.shape)
   # print(labels1.shape)

   train = np.load('data/mnist_rotation_new/rotated_train.npz','r+')
   valid = np.load('data/mnist_rotation_new/rotated_valid.npz','r+')
   test = np.load('data/mnist_rotation_new/rotated_test.npz','r+')
   # train = np.load('data/train.npz', 'r+')
   # valid = np.load('data/valid.npz', 'r+')
   # test = np.load('data/test.npz', 'r+')
